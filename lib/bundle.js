...
exports.bundle = function(moduleExpression, fileName, opts) {
  ...
  function bundleWatch() {
    ...
    .then(function(output) {
      ...
      }, function rebuild(triggerFile) {
        if (triggerFile != fileName &&
            triggerFile.indexOf('jspm.browser.js') == -1) {
          // skipping the bundle file
          return bundleOnly();
        } else {
          return Promise.resolve();
        }
...
function buildWatch(files, invalidate, build) {
  return new Promise(function(resolve, reject) {
    ...
    var watcher = sane(lowestDir, { glob: ['**/*.js', '**/*.json'], watchman: watchman });
    ...
    function changed(filepath) {
      ...
      if (filepath) {
        if(relFiles.indexOf(filepath) == -1) {
          ui.log('ok', 'File `' + file + '` changed, but not part of the bundle, skipping...');
        } else {
